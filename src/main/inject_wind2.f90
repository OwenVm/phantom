...
! Owen: Although some parts do not really "inject", it needs to be called inject such that it is called at the correct spot
! Owen: i.e., in evolve.f90, which calls inject.f90, and this one is called via initial.f90
subroutine init_inject(ierr)
 ...
 ! Owen: wind emitting sink = 1, i.e. sink particle 1
 ! Owen: iTeff etc are just defined in part, such that they refer to the correct quantity of the sink particle
...
! Owen: probably need to change this to a HE solution then, in a new file star.f90
 ...
 if (iwind_resolution == 0) then
    ...
    ! Owen: Probqbly wont need this with the new method 
    !
...
!compute full evolution (to get tcross) and save 1D profile for comparison
 if ( .not. pulsating_wind .or. nfill_domain > 0) then
   ! Owen: define tcross, i.e. the outer shell that needs to be set?
    ...
    if (tcross < 1.d98) then ! Owen: What does this do?
...
! Owen: This stuff is just for printing things to the output file/terminal
!-----------------------------------------------------------------------
...
!Same here, although some parts do not really "inject", it needs to be called inject such that it is called at the correct spot
!i.e., in evolve.f90, which calls inject.f90, and this one is called via initial.f90
subroutine inject_particles(time,dtlast,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,&
 ...
 if (nptmass > 0 .and. wind_emitting_sink <= nptmass) then
    x0 = xyzmh_ptmass(1:3,wind_emitting_sink) ! Owen: Position of the wind emitting sink
 ...
 if (npart > 0) then
    ...
    !
    inner_radius = wind_injection_radius + deltaR_osc*sin(omega_osc*time) ! Owen: Pretty sure this is just wind_injection_radius
    ...
    ! Owen: I don't think this does anythig of importance, given time_period is not used anywhere else?
    ...
    ! Owen: if nfill_domain neq 0, then initially already nfill_domain spheres are expelled
    nreleased = nfill_domain
    nboundaries = iboundary_spheres
    !release background particles
    ! Owen: released = false initially, at the end of this loop its true
    ! at the start of the simulation, release boundary particles.
    ! igas just means that in add_or_update_particle, the particle is added as a gas particle
    ipart = igas
    ...
    ! Owen: if npart = 0, i.e. at the very start of the simulation
...
! Owen: The index of every sphere is ever increasing in the simulation
 ...
 do i=inner_boundary_sphere,outer_sphere,-1
    ...
    !Owen: local_t = t + (nbs_spheres + nfill_domain - i) * time_between_spheres
    ...
    if (pulsating_wind.and.released) then
       ...
       !Owen: Uses the local time as a x index, to interpolate on the 'time' position of the sphere
          ...
          ! Owen: Interpolate wind profile to set values on the spheres
       endif
    ...
    if (i > inner_sphere) then
       ...
       ! Owen: If a sphere is beyond the inner sphere, then it will be injected
       if (idust_opacity == 2) then
       ...
       ! Owen: Now all particles that have previously been released, are updated, and ipart=igas is passed
       if (idust_opacity == 2) then
