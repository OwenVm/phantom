module inject
 ...
 real :: Rstar, Msink, Mtotal

 ! Unified shell arrays: indices 1..n_shells_bnd are boundary, rest are gas
 integer :: n_shells_bnd, n_shells_gas, n_shells_total
 integer, allocatable :: npart_per_shell(:)
 real,    allocatable :: shell_radii(:)
 real,    allocatable :: delta_r_shell(:)
...
subroutine init_inject(ierr)
 ...
 use wind_pulsating,only:setup_star,calc_stellar_profile,interp_stellar_profile,rho_power_fit
 use dust_formation,only:calc_kappa_max

 integer, intent(out) :: ierr
 real    :: Mstar_cgs, Rstar_cgs, Lstar_cgs
 real    :: current_radius, dr, r_c, rho, P, u, T
 integer :: shell_index, i, n_shell
 ...
 ! Build hydrostatic stellar profile starting at Rstar outward.
 ! rho_power_fit is set inside calc_stellar_profile from a log-log fit.
 call setup_star(Msink*umass, Rstar*au, r_max_on_rstar*Rstar*au, &
                 gmw, gamma, rho_inner, 0.0, T_inner, T_power_in)
 call calc_stellar_profile(n_profile_points)

 ! ----------------------------------------------------------------
 ! Build all shells in a single unified loop starting from Rstar.
 ! The first iboundary_spheres shells will be tagged as boundary type,
 ! the rest as gas — but the geometry and thermodynamics are identical.
 ! ----------------------------------------------------------------
 ...
 do
    ...
    if (shell_index == 1) then
       ...
       n_shell = max(1, nint(real(tmp_n(shell_index-1)) &
                   * (r_c / tmp_r(shell_index-1))**(-rho_power_fit + 2.0)))
    endif
 ...
 n_shells_gas   = n_shells_total - n_shells_bnd

 allocate(npart_per_shell(n_shells_total))
 allocate(shell_radii(n_shells_total))
 allocate(delta_r_shell(n_shells_total))

 do i = 1, n_shells_total
    npart_per_shell(i) = tmp_n(i)
    shell_radii(i)     = tmp_r(i)
    delta_r_shell(i)   = tmp_dr(i)
 enddo

 ! Particle mass from innermost shell using hydrostatic density
 call interp_stellar_profile(shell_radii(1), rho, P, u, T)
 mass_of_boundary_particle = 4.0*pi * rho * shell_radii(1)**2 * delta_r_shell(1) / real(n_particles_inner)
 ...
 print *, ''
 print *, ' rho_power (fitted from HSE)       :', rho_power_fit
 print *, ' rho_inner (cgs)                   :', rho_inner
 print *, ' T_inner (K)                       :', T_inner
 print *, ' T_power                           :', T_power_in
 print *, ' Profile/shells start / Rstar      :', 1.0
 ...
 print *, ' Gas shells                        :', n_shells_gas
 print *, ' Total shells                      :', n_shells_total
 print *, ' Total boundary particles          :', sum(npart_per_shell(1:n_shells_bnd))
 print *, ' Total gas particles               :', sum(npart_per_shell(n_shells_bnd+1:n_shells_total))
 print *, ' Innermost shell N_per_shell       :', npart_per_shell(1)
 print *, ' Outermost shell N_per_shell       :', npart_per_shell(n_shells_total)
 print *, ' Particle mass (code units)        :', mass_of_gas_particle
 print *, ''

 do i = 1, n_shells_total
    if (i <= n_shells_bnd) then
       print *, 'Boundary shell ', i, ': r/Rstar=', shell_radii(i)/Rstar, &
                ' dr/Rstar=', delta_r_shell(i)/Rstar, ' N=', npart_per_shell(i)
    else
       print *, 'Gas shell      ', i-n_shells_bnd, ': r/Rstar=', shell_radii(i)/Rstar, &
                ' dr/Rstar=', delta_r_shell(i)/Rstar, ' N=', npart_per_shell(i)
    endif
 enddo
...
subroutine setup_initial_atmosphere(xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,npart,npartoftype)
 ...
 integer :: i, j, first_particle, nboundary, ptype
 real    :: r, rho, P, u, T, x0(3), v0(3)
 ...
 do i = 1, n_shells_total
    ...
    if (i <= n_shells_bnd) then
       ptype = iboundary
       print *, 'BND shell', i, ': r/Rstar=', r/Rstar, ' T=', T, ' rho=', rho
    else
       ptype = igas
       print *, 'Gas shell', i-n_shells_bnd, ': r/Rstar=', r/Rstar, ' T=', T, ' rho=', rho
    endif

    first_particle = npart + 1
    call inject_fibonacci_sphere(i, first_particle, npart_per_shell(i), r, 0.0, u, rho, &
                                 npart, npartoftype, xyzh, vxyzu, ptype, x0, v0)
 enddo
...
subroutine reconstruct_boundary_info(time,xyzh,npart,xyzmh_ptmass)
 ...
 if (n_boundary_particles > 0) then
    ...
    enddo
    print *, 'Reconstructed boundary particle info, n_boundary=', n_boundary_particles
 endif
...
subroutine apply_pulsation(time,xyzh,vxyzu,npart,xyzmh_ptmass,vxyz_ptmass)
 ...
 if (time_puls > 0 .and. time < pulsation_period * time_puls) then
...
subroutine take_periodic_mass_measurements(time,xyzh,npart,xyzmh_ptmass,npartoftype)
 use part,   only:igas,iboundary,iphase,iamtype
 use physcon,only:years,days

 real,    intent(in) :: time
 real,    intent(in) :: xyzh(:,:),xyzmh_ptmass(:,:)
 integer, intent(in) :: npart, npartoftype(:)

 real    :: current_mass, mass_lost, x0(3), dx, dy, dz, r, sum_rates, sink_mass
 integer :: i
 ...
 if (measurement_active .and. time >= time_next_measurement .and. time < mass_loss_end_time) then
    x0        = xyzmh_ptmass(1:3, wind_emitting_sink)
    sink_mass = xyzmh_ptmass(4,   wind_emitting_sink)
    current_mass = sink_mass
    do i = 1, npart
       dx = xyzh(1,i)-x0(1); dy = xyzh(2,i)-x0(2); dz = xyzh(3,i)-x0(3)
       r  = sqrt(dx**2+dy**2+dz**2)
       if (r <= mass_loss_check_radius) then
          if (iamtype(iphase(i)) == iboundary) then
             current_mass = current_mass + mass_of_boundary_particle
          else
             current_mass = current_mass + mass_of_gas_particle
          endif
       endif
    enddo
    mass_lost = mass_previous_measurement - current_mass
    n_measurements = n_measurements + 1
    mass_loss_rates(n_measurements) = mass_lost / measurement_interval
    mass_previous_measurement = current_mass
    time_next_measurement     = time + measurement_interval
 endif
...
subroutine check_continuous_reinject(time,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,npart,npartoftype)
 ...
 print *, 'Time since last reinject (days): ', (time - time_last_reinject)*utime/days
 print *, '-----------------------------------------'
 print *, ''
...
subroutine perform_reinjection(time,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,npart,npartoftype)
 ...
 integer :: old_npart
 real    :: r_inject, phase, r_dot, rho, P, u, T, x0(3), v0(3), mass_injected
 ...
 ! Inject just outside the outermost boundary shell
 if (allocated(r_boundary_equilibrium) .and. n_boundary_particles > 0) then
    r_inject = r_boundary_equilibrium(n_boundary_particles) + delta_r_shell(n_shells_bnd + 1)
 else
    r_inject = shell_radii(n_shells_bnd + 1)
 endif
 ...
 print *, ' Particles injected :', (npart - old_npart), '  total:', npart
...
subroutine read_mass_loss_data()
 ...
 read(iunit,*)
 read(iunit,*,iostat=ierr) mass_loss_rate_calculated;  if (ierr /= 0) then; close(iunit); return; endif
...
!-----------------------------------------------------------------------
subroutine calculate_period(M, R, period_days)
 real, intent(in)  :: M, R
 real, intent(out) :: period_days

 period_days = 10.0**(-1.92 - 0.73*log10(M) + 1.86*log10(R * 215.032))
 print *, 'Calculated pulsation period (days): ', period_days
...
subroutine write_options_inject(iunit)
 ...
 call write_inopt(T_inner,              'T_inner',             'temperature at Rstar (K)',iunit)
 call write_inopt(T_power_in,           'T_power',             'temperature profile exponent: T ~ r^(-T_power)',iunit)
 call write_inopt(r_max_on_rstar,       'r_max_on_rstar',      'atmosphere outer boundary (fraction of R_star)',iunit)
 call write_inopt(rho_inner,            'rho_inner',           'density at Rstar (cgs)',iunit)
 call write_inopt(iwind,                'iwind',               'wind type: 1=prescribed, 2=period from M-R relation',iunit)
 ...
 call write_inopt(time_puls,            'time_puls',           'ramp-up time in periods (-1=instant)',iunit)
 ...
 call write_inopt(var_boundary,         'var_boundary',        'update boundary thermo with pulsation',iunit)
 call write_inopt(reinject_enabled,     'reinject_enabled',    'enable dynamic reinjection',iunit)
