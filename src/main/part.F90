...
!
 use dim, only:ndim,maxp,maxpsph,maxsts,ndivcurlv,ndivcurlB,maxvxyzu,maxalpha,&
               ...
               inucleation,itau_alloc,itauL_alloc,icool_alloc,use_apr,apr_maxlevel,maxp_apr,maxptmassgr,&
               use_sinktree
...
subroutine allocate_part
 ...
 call allocate_array('cool_rate', cool_rate, maxp*icool_alloc)
 if (use_krome) then
...
subroutine copy_particle(src,dst,new_part)
 ...
 if (icool_alloc == 1) cool_rate(dst) = cool_rate(src)
...
!--NB: could use MPI_PACK here...
 if (i > 0) then
    ...
    if (icool_alloc == 1) call fill_buffer(xtemp, cool_rate(i), nbuf)
...
subroutine unfill_buffer(ipart,xbuf)
 ...
 if (icool_alloc == 1) cool_rate(ipart) = unfill_buf(xbuf, j)
 if (maxgrav==maxp) then
